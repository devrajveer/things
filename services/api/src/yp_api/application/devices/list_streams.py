...
from yp_api.domain.devices.entities import Stream
...
class ListStreamsUseCase:
    ...
    async def execute(self, actor_id: str, project_id: str, device_id: Optional[str] = None) -> List[Stream]:
        async with self.uow:
            # 1. Authorization
            ...
            # 2. List
            if device_id:
                return await self.uow.streams.list_by_device(device_id)
            return await self.uow.streams.list_by_project(project_id)
